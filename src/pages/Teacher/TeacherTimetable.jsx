...
export default function TeacherTimetable() {
  ...
  // Read timetable from admin
  const entries = JSON.parse(localStorage.getItem("timetable") || "[]");

  // Filter by teacher name OR department
  const myEntries = useMemo(() =>
    entries.filter((e) =>
      e.teacher?.toLowerCase() === teacher.name?.toLowerCase() ||
  ...
  const uniqueSubjects = [...new Set(myEntries.map((e) => e.subject))];
      ...
      <div style={{ background: "linear-gradient(135deg,#0F172A,#1E293B)", borderRadius: "16px", padding: "20px 24px", marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          ...
          <p style={{ color: "#64748B", margin: "4px 0 0", fontSize: "0.82rem" }}>
            {teacher.department} · {totalClasses} class slot{totalClasses !== 1 ? "s" : ""} per week
          </p>
        </div>
      ...
      {myEntries.length === 0 ? (
        <div style={{ textAlign: "center", padding: "3rem", background: "#7DD3FC", borderRadius: "14px", color: "#0369A1" }}>
          ...
          <p style={{ fontSize: "0.85rem", opacity: 0.8 }}>
            The admin will assign your teaching schedule. Once added, it will appear here.
          </p>
        </div>
      ) : (
        <>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                {todayEntries.map((e) => {
                  ...
                  return (
                    <div key={e.id} style={{ background: col.bg, border: `1px solid ${col.border}`, borderRadius: "10px", padding: "12px 16px", minWidth: "180px" }}>
                      <p style={{ fontWeight: "700", color: col.text, margin: "0 0 3px", fontSize: "0.9rem" }}>{e.subject}</p>
                      <p style={{ color: col.text, fontSize: "0.78rem", margin: "0 0 2px", opacity: 0.85 }}>⏰ {e.timeSlot}</p>
                      {e.classSection && <p style={{ color: col.text, fontSize: "0.78rem", margin: "0 0 2px", opacity: 0.85 }}>🏫 {e.classSection}</p>}
                        ...
                        <td key={day} style={{ padding: "6px 8px", textAlign: "center", verticalAlign: "top", minWidth: "120px", border: "1px solid rgba(14,165,233,0.15)", background: day === today ? "rgba(56,189,248,0.05)" : "transparent" }}>
                          {cell ? (
                            <div style={{ background: col.bg, border: `1px solid ${col.border}`, borderRadius: "8px", padding: "6px 8px", textAlign: "left" }}>
                              ...
                              {cell.classSection && <p style={{ margin: 0, color: col.text, fontSize: "0.7rem", opacity: 0.85 }}>🏫 {cell.classSection}</p>}
                              {cell.room && <p style={{ margin: "2px 0 0", color: col.text, fontSize: "0.7rem", opacity: 0.85 }}>📍 {cell.room}</p>}
                            </div>
                          ) : (
            ...
            <div style={{ marginTop: "1.25rem", background: "#7DD3FC", borderRadius: "12px", padding: "14px 18px", border: "1px solid rgba(14,165,233,0.2)" }}>
              ...
              <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                {uniqueSubjects.map((subject) => {
                  const col = subjectColors[subject] || SLOT_COLORS[0];
                  return (
